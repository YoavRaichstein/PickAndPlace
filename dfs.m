  % function finalList = dfs(A, start, goal)
  % 
  % Function: dfs 
  % 
  % Description: Implamantation of Depth first algorithm 
  % 
  % Parameters: 
...
while ~isempty(OpenList) && ~GoalReached % while there are nodes left to explore and the goal was not reached do...
    ...
    for i = 1:size(u,1) % iterate through possible actions (right up left down)
        ...
        if A(newState(2),newState(1)) == 1
            ...
            visitedCount_dfs(end) = visitedCount_dfs(end)+1;
...
finalCount_dfs(end+1) = size(finalList, 1)
disp(visitedCount_dfs)
%save('test_dfs.mat', 'visitedCount_dfs', 'finalCount_dfs') %saving the results
end