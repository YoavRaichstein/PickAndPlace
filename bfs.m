  ...
  % function finalList = bfs(A, start, goal)
  % 
  % Function: bfs 
  % 
  % Description: Implamantation of Breadth first algorithm 
  % 
  % Parameters: 
...
while ~isempty(OpenList) && ~GoalReached % while there are nodes left to explore and the goal was not reached do...
    ...
    for i = 1:size(u,1) % iterate through possible actions (right up left down)
        ...
        if A(newState(2),newState(1)) == 1
            ...
            visitedCount_bfs(end) = visitedCount_bfs(end)+1;
...
finalCount_bfs(end+1) = size(finalList, 1)
disp(visitedCount_bfs)
%save('test_bfs.mat', 'visitedCount_bfs', 'finalCount_bfs') %saving the results
end