  % function finalList = A2(A, start, goal)
  % 
  % Function: A2
  % 
  % Description: Implamantation of A* algorithm focusing on the cost
  % 
  % Parameters: 
...
while ~isempty(OpenList) && ~GoalReached % while there are nodes left to explore and the goal was not reached do...
    ...
    for i = 1:size(u,1) % iterate through possible actions (right up left down)
        ...
        if A(newState(2),newState(1)) == 1
            ...
            visitedCount_A2(end) = visitedCount_A2(end)+1;
...
finalCount_A2(end+1) = size(finalList, 1)
disp(visitedCount_A2)
%save('test_A2.mat', 'visitedCount_A2', 'finalCount_A2') %saving the results
end